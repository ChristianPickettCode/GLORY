...
def train(model, optimizer, scaler, scheduler, dataloader, device, cfg, early_stopping):
    model.train()
    ...
    for cnt, (subgraph, mapping_idx, candidate_news, candidate_entity, entity_mask, labels) \
            in enumerate(tqdm(dataloader,
                              ...
                              desc=f"[{device}] Training"), start=1):
        ...
        with amp.autocast(enabled=cfg.get("use_amp", False) and torch.cuda.is_available()):
        ...
        if cnt % cfg.log_steps == 0:
            if device == 0:  # Only log from the main process
            ...
            print('[{}] Ed: {}, average_loss: {:.5f}, average_acc: {:.5f}'.format(
                device, cnt * cfg.batch_size, sum_loss.item() / cfg.log_steps, sum_auc.item() / cfg.log_steps))
            sum_loss.zero_()
            sum_auc.zero_()
        
        if cnt > int(cfg.val_skip_epochs * (cfg.dataset.pos_count // cfg.batch_size + 1)) and cnt % cfg.val_steps == 0:
            res = val(model, device, cfg)
            model.train()
...
def val(model, device, cfg):
    ...
    reduced_auc = reduce_mean(torch.tensor(np.nanmean(val_auc)).float().to(device), cfg.gpu_num)
    reduced_mrr = reduce_mean(torch.tensor(np.nanmean(val_mrr)).float().to(device), cfg.gpu_num)
    reduced_ndcg5 = reduce_mean(torch.tensor(np.nanmean(val_ndcg5)).float().to(device), cfg.gpu_num)
    reduced_ndcg10 = reduce_mean(torch.tensor(np.nanmean(val_ndcg10)).float().to(device), cfg.gpu_num)
...
def main_worker(local_rank, cfg):
    ...
    dist.init_process_group(backend='gloo',  # Change backend to 'gloo'
    ...
    optimizer.zero_grad(set_to_none=True)
    scaler = amp.GradScaler(enabled=cfg.get("use_amp", False) and torch.cuda.is_available())
    ...
    train(model, optimizer, scaler, scheduler, train_dataloader, device, cfg, early_stopping)
...
def main(cfg: DictConfig):
    ...
    print("GPUS detected by torch:", cfg.gpu_num)
    if cfg.gpu_num == 0:
        cfg.gpu_num = 1
        print("No GPUs detected. Falling back to CPU execution.")
